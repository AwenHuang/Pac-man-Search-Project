...
def cornersHeuristic(state, problem):
  ...
  for corner in corners:
    ...
    if not visited:
      #sum = sum + ( (coor[0]-corner[0]) ** 2 + (coor[1]-corner[1]) ** 2) ** 0.5
      #sum = sum + fabs(coor[0] - corner[0]) + fabs(coor[1] - corner[1])
      if(iswall==0):
        sum = sum + ( (coor[0]-corner[0]) ** 2 + (coor[1]-corner[1]) ** 2) ** 0.5
      elif(iswall > notwall):
        sum = sum + fabs(coor[0] - corner[0]) + fabs(coor[1] - corner[1])+iswall
      else:
        sum = sum + fabs(coor[0] - corner[0]) + fabs(coor[1] - corner[1])
